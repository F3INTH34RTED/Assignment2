...
void Execute_On_Cpu(unsigned int array_size_percentage, int processing_part, unsigned long int *input_array_a, unsigned long int *input_array_b, unsigned long int *result_array_c);
...
{
    // INITIALIZE ARRAYS
    unsigned long int *input_array_a = (int *)malloc(sizeof(int) * ARRAY_SIZE);
    unsigned long int *input_array_b = (int *)malloc(sizeof(int) * ARRAY_SIZE);
    unsigned long int *result_array_c = (int *)malloc(sizeof(int) * ARRAY_SIZE);
    ...
    for (int i = 0; i < ARRAY_SIZE; i++)
    {
        ...
        {
            input_array_a[i] = i;
            input_array_b[i] = C_ARRAY_SIZE - 1 - i;
            result_array_c[i] = 1;
        }
    }

    Execute_On_Cpu(50, FIRST_HALF_OF_ARRAY, input_array_a, input_array_b, result_array_c);
...
void Execute_On_Cpu(unsigned int array_size_percentage, int processing_part, unsigned long int *input_array_a, unsigned long int *input_array_b, unsigned long int *result_array_c)
{
    ...
    Display_Specific_Platform_Info(platforms, platform_index);
    ...
    */
    ...
    Display_Specific_Device_Info(devices, device_index);
    ...
    // CREATE KERNEL
    cl_kernel kernel = clCreateKernel(program, "kernel_exponentiation", &status);
    if (status != CL_SUCCESS)
    {
    ...
    // CREATE QUEUE
    cl_command_queue commandQ = clCreateCommandQueueWithProperties(context, devices[device_index], 0, &status);
    if (status != CL_SUCCESS)
    {
    ...
    size_t local_size = 10, global_size = (size_t)ceil(C_ARRAY_SIZE / (float)local_size) * local_size;
    printf("Local size: %zu\nGlobal size: %zu\n", local_size, global_size);
    system("pause");
    system("cls");

    // CREATE BUFFER
    cl_mem clmem_input_array_a = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(unsigned long int) * C_ARRAY_SIZE, NULL, &status);
    if (status != CL_SUCCESS)
    {
    ...
    cl_mem clmem_input_array_b = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(unsigned long int) * C_ARRAY_SIZE, NULL, &status);
    if (status != CL_SUCCESS)
    {
        printf("Could not create buffer for result array.\n");
    ...
    cl_mem clmem_result_array_c = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(unsigned long int) * C_ARRAY_SIZE, NULL, &status);
    if (status != CL_SUCCESS)
    {
        printf("Could not create buffer for result array.\n");
        clReleaseMemObject(clmem_input_array_a);
    ...
    status = clEnqueueWriteBuffer(commandQ, clmem_input_array_a, CL_TRUE, 0, ARRAY_SIZE * sizeof(unsigned long int), input_array_a, 0, NULL, NULL);
    if (status != CL_SUCCESS)
        printf("Could not write input array a buffer to the device.\n");
    else printf("Buffer a written successfully!\n");

    status = clEnqueueWriteBuffer(commandQ, clmem_input_array_b, CL_TRUE, 0, ARRAY_SIZE * sizeof(unsigned long int), input_array_b, 0, NULL, NULL);
    if (status != CL_SUCCESS)
        printf("Could not write input array b buffer to the device.\n");
    else printf("Buffer a written successfully!\n");

    status = clEnqueueWriteBuffer(commandQ, clmem_result_array_c, CL_TRUE, 0, ARRAY_SIZE * sizeof(unsigned long int), result_array_c, 0, NULL, NULL);
    if (status != CL_SUCCESS)
    ...
    // READ FROM DEVICE
    status = clEnqueueReadBuffer(commandQ, clmem_result_array_c, CL_TRUE, 0, ARRAY_SIZE * sizeof(int), result_array_c, 0, NULL, NULL);
    if (status != CL_SUCCESS)
